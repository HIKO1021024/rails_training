...
class InfosControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create info" do
    assert_difference('Info.count') do
      post infos_url, params: { info: { info: @info.info, title: @info.title, user_id: @info.user_id } }
    end
  ...
  test "should update info" do
    patch info_url(@info), params: { info: { info: @info.info, title: @info.title, user_id: @info.user_id } }
